...
export default function ResultsDisplay({
  ...
  const savingsPercentage = Math.round(
    ((results.rentMonthlyCost - results.buyMonthlyCost) /
      results.rentMonthlyCost) *
      100
  );
      ...
      >
        ...
        >
          ...
          <span style={{ color: "#fff", fontWeight: "bold" }}>
            {Math.abs(savingsPercentage)}% cheaper
          </span>{" "}
          than {isBuyingCheaper ? "Renting" : "Buying"} on monthly basis
        </Typography>
        <Typography
          sx={{ ...typographyStyles.bodyMedium, color: "#fff", mb: 3 }}
        >
          Buying uses {Math.abs(results.capitalDifference)}%{" "}
          {results.capitalDifference < 0 ? "less" : "more"} capital than renting
          if you stay in Dubai for {results.stayDuration} years.
        </Typography>

        {/* Monthly Cost Comparison */}
        <Box sx={{ height: "300px", mb: 8 }}>
          <ComparisonChart
            buyMonthlyCost={results.buyMonthlyCost}
            rentMonthlyCost={results.rentMonthlyCost}
          />
        </Box>
        ...
        <Box sx={{ mb: 3 }}>
          <Tabs
            ...
            onChange={(_, newValue) =>
              onBreakdownChange(newValue === 0 ? "rent" : "buy")
            }
            textColor="inherit"
        ...
        {activeBreakdown === "rent" ? (
          <Box>
            ...
            >
              <Typography
                sx={{ ...typographyStyles.bodyMedium, color: "#fff" }}
              >
                Monthly Rent
              </Typography>
              <Typography
